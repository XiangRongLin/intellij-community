...
class KotlinIntroduceConstantHandler(
    ...
    private fun validateElements(elements: List<PsiElement>): String? {
        ...
        return when {
            ...
            elements.any { it.isNotConst() } -> KotlinBundle.message("error.text.can.t.introduce.constant.for.this.expression.because.not.constant")
            else -> null
        }
    }

    private fun PsiElement.isNotConst() = !isConst()
        ...
        if (this is KtBinaryExpression) {
            ...
            object : KotlinRecursiveElementVisitor() {
                override fun visitKtElement(element: KtElement) {
                    ...
                    if (element !is KtOperationReferenceExpression && element !is KtBinaryExpression) {
